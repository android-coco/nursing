...
func QueryIntakeOrOutputVolume(patientId string, tp int, page int) ([]IntakeOutputDup, error) {
	...
	idx := page * count
	err := fit.MySqlEngine().Omit("id", "date_time").Table("intake_output").Where("patient_id = ? and type = ?", patientId, tp).Limit(count, idx).Find(&responseObj)
...
func QueryIntakeOrOutputVolumeAll(patientId string, page int) ([]IntakeOutputDup, error) {
	...
	responseObj := make([]IntakeOutputDup, 0)
	idx := page * count
	err := fit.MySqlEngine().Omit("id", "date_time").Table("intake_output").Where("patient_id = ?", patientId).And("(type = ? or type = ?)", IntakeOutputTypeOutput, IntakeOutputTypeIntake).Limit(count, idx).Find(&responseObj)
	return responseObj, err
}
