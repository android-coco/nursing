...
func GetPatientInfo(patientId string) ([]PatientInfo, error) {
	...
	patient := VAA1Dup{}
	// 查询病人详情
	_, err := fit.MySqlEngine().SQL("select VAA01, VAA04, VAA05, ABW01, VAA10, BDP02, BCK01B, BCQ04, VAA73, ABQ02 from VAA1 where VAA01 = ? order by VAA01 desc", patientId).Get(&patient)
	...
	if err == nil && patient.VAA01 != 0 {
		...
		_, err = fit.SQLServerEngine().SQL("select VAE1.BCE03B, VAE1.BCE03C, BBY1.BCF01 as AAG01 from VAE1, BBY1 where VAE1.BCK01C = ? and VAE1.VAE44 in ('1','2') and VAE1.VAA01 = ? and BBY1.BBY01 = VAE1.AAG01 order by VAE1.VAE01 desc", patient.BCK01C, patient.VAA01).Get(&patientInfo)
...
/*查询住院期间的手术记录  pid: 病人ID， date：最近一次住院的住院日期*/
func FetchOperationRecordsDuringHospitalization(pid int64, date string) ([]VAT1, error) {
	records := make([]VAT1, 0)
	// VAT04 = 4 表示已结束手术
	err := fit.SQLServerEngine().SQL("select VAA01, VAT08 from VAT1 where VAA01 = ? and VAT08 > ? and VAT04 = 4", pid, date).Find(&records)
	return records, err
}
