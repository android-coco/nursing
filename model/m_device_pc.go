...
/*医嘱拆分查询*/
func SearchMedicalAdvicesForSplitting(startTime, endTime, pidArr, printTypeArr string, typeOf, print int) ([]MedicalAdvicePrint, error) {
...
func SearchMedicalAdviceExecutionForPC(typeOf, status int, category, pids, st, et string) ([]PCMedicalAdviceExec, error) {
	condition_type := ""
	if typeOf != 0 {
		// 长期和临时
...
func SearchMedicalAdviceForPC(typeOf, status int, category, pids, st, et string) ([]PCMedicalAdvice, error) {
	condition_type := ""
	if typeOf != 0 {
		// 长期和临时
		condition_type = fmt.Sprintf(" and a.VAF11 = '%d'", typeOf)
