...
type BCE1 struct {
	...
	BCE19 string //毕业时间, graduated date
	ABU02 string //ADIVN
	AAD01 string //所学专业
	...
	AAH01 string //聘任职务
	BCE28 string //工作日期, joindate
	AAQ01 string //执业类别
	ABE01 string //执业范围, 需要单独的表
	BCE31 string //进院日期, HireDate
	BCE32 string //离职日期, DimissionDate
...
type VAA1 struct {
	...
	VAA57  string  // 就诊时间, LastVisitDate
...
func synchronizingBCE1() {
	...
	for _, k := range mods {
		fit.MySqlEngine().Exec("delete from BCE1 where BCE01 = ?", k.BCE01)
		fit.MySqlEngine().Table("BCE1").InsertOne(&k)
	}
	timeNow := time.Now()
...
func synchronizingVAA1() {
	...
	for _, k := range mods {
		fit.MySqlEngine().Exec("delete from VAA1 where VAA01 = ?", k.VAA01)
		fit.MySqlEngine().Table("VAA1").InsertOne(&k)
	}
	timeNow := time.Now()
	timeDur := timeNow.Sub(timeMark)
